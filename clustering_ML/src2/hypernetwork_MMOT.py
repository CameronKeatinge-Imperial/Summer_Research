...
class MMOTHypernetworkObject(BaseHypernetworkObject):
    ...
    def update_neighbourhood_scores(self, removed_nodes=None, depth=2) -> list:
        ...
        for _ in range(int(depth)):
            ring = set()
            for n in frontier:
                ring |= self._node_to_edges.get(n, set())
            ring -= affected
            if not ring:
                break
            affected |= ring
            ...
            nxt = set()
            for eid in ring:
                nxt |= set(self._edge_to_nodes[eid])
