...
class DataObject(ABC):
    ...
    def cluster_size_terminate(self, target_cluster_no):
        ...
        if target_cluster_no == 2:
            if frac[0] < 3 / 5:
                print(f"Terminating due to largest cluster less than size 3 / 5, as target_cluster = 2")
                return True
            if sum(frac[:(target_cluster_no+1)]) < 3 / 4:
                print(f"Terminating due to twp largest clusters less than size 3 / 4")
                return True            
        else:
