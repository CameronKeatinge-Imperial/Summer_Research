...
class EvaluateClustering():
    def __init__(self,config,true_values,predicted_clusters):
        ...
        # NEEDS GENERALISED #
        #####################
        mapping = self.match_labelling_of_true(self.true_values)   # e.g. {0: 'Fail', 1: 'Pass', 2: 'Middle'}
        group_labels = self.partitions_to_list(mapping=mapping)
...
def partitions_to_list(self, mapping=None):
    ...
    def match_labelling_of_true(self, true_labels, labels=None):
        '''
        ...
        true_labels : list-like, one true label per sample, in the same order/
                    indexing as self.predicted_clusters (1-based positions).
        labels      : optional explicit list of true label categories, e.g.
                    ['Pass', 'Middle', 'Fail']. If None, inferred as the
                    sorted set of unique values in true_labels.
        ...
        # If n_clusters > n_labels, some clusters won't get an assignment from
        # linear_sum_assignment (it's limited to min(n_labels, n_clusters) pairs).
        # Fall back to "best matching label" for any leftover clusters.
