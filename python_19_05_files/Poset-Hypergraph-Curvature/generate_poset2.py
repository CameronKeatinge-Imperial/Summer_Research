...
# Read the second input file as a continuous stream of tokens to extract the canonical 315 hyperedges
with open(sys.argv[2],'r') as f:
  raw_tokens = f.read().split()
all_node_ids = list(map(int, raw_tokens))

total_expected_edges = 315
avg_elements_per_edge = len(all_node_ids) // total_expected_edges

pointer = 0
for i in range(total_expected_edges):
  if i < (len(all_node_ids) % total_expected_edges):
    window = avg_elements_per_edge + 1
  else:
    window = avg_elements_per_edge
    
  edge_nodes = all_node_ids[pointer : pointer + window]
  pointer += window
  
  nodes[count] = set(edge_nodes)
